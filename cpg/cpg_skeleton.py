...
"""
CPG NMT skeleton, use with cpg_vocab.py
A very basic implementation of neural machine translation

Usage:
    nmt.py train --train-src-list=<file> --train-tgt-list=<file> --dev-src-list=<file> --dev-tgt-list=<file> --vocab=<file> [options]
...
Options:
    ...
    --cuda                                  use GPU
    --train-src-list=<file>                 train source file
    --train-tgt-list=<file>                 train target file
    --dev-src-list=<file>                   dev source file
    --dev-tgt-list=<file>                   dev target file
...
class NMT(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # for lang, vocabentry in self.vocab.src.items():
...
def train(args: Dict[str, str]):
    ...
    train_corpus_src = read_list(args['--train-src-list'])
    train_corpus_tgt = read_list(args['--train-tgt-list'])

    # [ train_data_src1, train_data_src2 ]
    # [ train_data_tgt1, train_data_tgt2 ]
    # zip => [ (train_data_src1, train_data_tgt1), (train_data_src2, train_data_tgt2)]
    train_datas_src = [read_iwslt_corpus(corpus_src, source="src")
                       for corpus_src in train_corpus_src]

    train_datas_tgt = [read_iwslt_corpus(corpus_tgt, source="tgt")
                       for corpus_tgt in train_corpus_tgt]
    print("reading dev datas...")
    dev_corpus_src = read_list(args['--dev-src-list'])
    dev_corpus_tgt = read_list(args['--dev-tgt-list'])

    dev_datas_src = [read_iwslt_corpus(corpus_src, source="src")
                     for corpus_src in dev_corpus_src]
    dev_datas_tgt = [read_iwslt_corpus(corpus_tgt, source="tgt")
                     for corpus_tgt in dev_corpus_tgt]

    train_datas = list(zip(train_datas_src, train_datas_tgt))
    dev_datas = list(zip(dev_datas_src, dev_datas_tgt))
